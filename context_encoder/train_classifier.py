...
def main():
    ...
    ax.plot(train_losses_model[0], color='red', linestyle='-', label='Unfrozen backbone - train')
    ax.plot(train_losses_model[1], color='blue', linestyle='-', label='Frozen backbone - train')
    ax.plot(val_losses_model[0], color='red', linestyle='--', label='Unfrozen backbone - validation')
    ax.plot(val_losses_model[1], color='blue', linestyle='--', label='Frozen backbone - validation')
    ax.set_xlabel('Epochs')
    ax.set_ylabel('Loss')
    ax.set_title('Training and Validation Loss')
    ax.legend()
    ax = fig.add_subplot(1,2,2)
    ax.plot(val_accuracies_model[0], color='red', linestyle='-', label='Unfrozen backbone')
    ax.plot(val_accuracies_model[1], color='blue', linestyle='-', label='Frozen backbone')
    ax.set_xlabel('Epochs')
